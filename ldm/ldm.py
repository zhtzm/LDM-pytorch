...
class LDM(DDPM):
    ...
    def losses(self, x: torch.Tensor, condition: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        ...
        model_out = self.wrapper(x_t, t, condition)

        return self.get_loss(noise, model_out)
    ...
    def denoising_loop(self, shape: tuple, condition: torch.Tensor, return_intermediates: bool = False):
        ...
        for i in tqdm(reversed(range(0, self.t_max)), desc='Sampling t', total=self.t_max):
            ...
            if (i % self.sample_img_every_t == 0 or i == self.t_max - 1) and return_intermediates:
    ...
    def sample(self, batch_size: int, return_intermediates: bool, condition: list = None) -> torch.Tensor:
        ...
        intermediates = [self.ae_model.decode(latent_intermediate) for latent_intermediate in latent_intermediates] \
